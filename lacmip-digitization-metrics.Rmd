...
```
A total of **`r format(totalLots, 0)` specimen lots were digitized during the `r project` project**. Not all specimens were digitized, mostly due to poor quality of the original specimen. We can see a breakdown of lots processed by digitization status and collection area from the results of the function above:
...
```
The `r project` project digitization rate is graphed below by the number of lots processed over time, an **average of `r round(averageLots,0)` specimen lots per month**.
...
```
The average time per specimen lot per task is graphed below.
```{r echo = FALSE, message = FALSE}
digitizationTask(data)
graphDigitizationTaskAvg
```
The average time to process a specimen lot including all tasks is graphed below.
```{r}
graphDigitizationTaskTime_total
```
A comparion between tasks over time is graphed below.
```{r}
