...
lots(data)
plotLot <- read_csv("output_lots.csv", na = character())
legendText <- c(paste("Digitized - Stratigraphic (", plotLot$`%`[1], "%)", sep = ""), paste("Digitized - Taxonomic (", plotLot$`%`[2], "%)", sep = ""), paste("not digitized (", plotLot$`%`[3], "%)", sep = ""))
ggplot(plotLot, aes(x = "", y = count, fill = `lot type`)) + 
...
![Digitization is comprised of specific tasks, often completed by people with different roles in the project (staff, student, volunteer, etc.). LACMIP identified and tracked six specific digitization tasks, and the average time required to complete each task for one specimen lot is graphed here.](output_digitizationTaskAvg.jpg)

![The digitization rate over time broken up by task is graphed here](output_digitizationTaskTime_task.jpg)

### What factors effected digitization rates?

![The average time to digitize a single specimen lot (all tasks included) varied, as shown in the graph here, with no clear indication that overall project duration affected digitization rate.](output_digitizationTaskTime_total.jpg)